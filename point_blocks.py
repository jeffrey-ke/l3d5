...
class InputTransform(nn.Module):
    def __init__(self, args=None, dim=3):
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(dim,64,(1,1),1), # don't need any padding probably because it's a 1x1 kernel
            nn.ReLU(),
            nn.BatchNorm2d(64),
        )
        ...
        self.to_transform = nn.Linear(256, dim**2)
        def init(m):
    ...
    def forward(self, X): # X is shape B,N,dim
        B,N,*_ = X.shape
        X = X.unsqueeze(-1)
        X = X.permute((0,2,1,3)) # shape is B,dim,N,1
        X = self.pointwise_mlps(X)
        X = self.maxpool2d(X) # collapses (B,1024,N,1) to (B,1024,1) probably
        X = X.view(B,-1)
        X = self.fc512(X)
        X = self.fc256(X)
        iden = torch.eye(self.dim, device=self.device).flatten()
        X = self.to_transform(X) + iden
        return X.view(B,self.dim,self.dim)
